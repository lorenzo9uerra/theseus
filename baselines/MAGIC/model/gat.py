...
class GATConv(nn.Module):
    def __init__(
        ...
        if isinstance(in_dim, tuple):
            ...
            self.fc_dst = nn.Linear(
                self.dst_feat, self.out_feat * self.n_heads, bias=False
            )
        else:
            self.fc_node_embedding = nn.Linear(
                self.src_feat, self.out_feat * self.n_heads, bias=False
        ...
        self.edge_fc = nn.Linear(
            self.edge_feat, self.out_feat * self.n_heads, bias=False
        )
        self.attn_h = nn.Parameter(
            torch.FloatTensor(size=(1, self.n_heads, self.out_feat))
        )
        self.attn_e = nn.Parameter(
            torch.FloatTensor(size=(1, self.n_heads, self.out_feat))
        )
        self.attn_t = nn.Parameter(
            torch.FloatTensor(size=(1, self.n_heads, self.out_feat))
        )
    ...
    def forward(self, graph, feat, get_attention=False):
        ...
        with graph.local_scope():
            ...
            graph.update_all(fn.u_mul_e("hs", "a", "m"), fn.sum("m", "hs"))
