...
def main(main_args):
    ...
    # Create strict validation labels from the shared attack/contaminated metadata.
    # Attack nodes remain positive; contaminated nodes are masked out of model
    # selection rather than treated as positives or negatives.
    y_val = np.zeros(val_node_offset)
    val_keep_mask = np.ones(val_node_offset, dtype=bool)
    ...
    if filter_process_only and val_process_masks:
        ...
        for idx in remapped_attack:
            if idx < val_node_offset:
                y_val[idx] = 1.0
        for idx in remapped_contaminated:
            if idx < val_node_offset:
                val_keep_mask[idx] = False

        print(
            "  Validation labels (process-only, strict): "
            f"{len(remapped_attack)} attack, {len(remapped_contaminated)} contaminated masked"
        )
    else:
        ...
        for idx in val_contaminated_idx:
            if idx < val_node_offset:
                val_keep_mask[idx] = False
        print(
            "  Validation labels (strict): "
            f"{len(val_attack_idx)} attack, {len(val_contaminated_idx)} contaminated masked"
        )

    n_val_eval = int(val_keep_mask.sum())
    n_val_attack = int(y_val[val_keep_mask].sum())
    n_val_benign = n_val_eval - n_val_attack

    print("\nTraining/Validation Setup:")
    print(f"  Training graphs: {n_train_graphs}")
    print(
        "  Validation nodes used for model selection: "
        f"{n_val_eval} ({n_val_benign} benign, {n_val_attack} attack)"
    )
    ...
    # Early stopping setup based on validation average precision.
    best_val_ap = 0.0
    patience_counter = 0
    best_model_path = os.path.join(
        _MAGIC_ROOT,
        "checkpoints",
        f"checkpoint-{dataset_name}-seed{main_args.seed}{_run_suffix(main_args)}-best.pt",
    )
    ...
    for epoch in epoch_iter:
        ...
        # Validation phase - compute AP on labeled validation set
        model.eval()
        with torch.no_grad():
            ...
            # Compute strict AP on validation set with contaminated nodes masked out.
            val_scores_eval = val_scores[val_keep_mask]
            y_val_eval = y_val[val_keep_mask]
            if y_val_eval.sum() > 0:
                val_ap = average_precision_score(y_val_eval, val_scores_eval)
            else:
                val_ap = 0.0

        epoch_iter.set_description(
            f"Epoch {epoch} | train_loss: {train_loss:.4f} | val_AP: {val_ap:.4f} | "
            f"best: {best_val_ap:.4f} | patience: {patience_counter}/{main_args.patience}"
        )

        # Check for improvement (higher AP is better)
    ...
    if os.path.exists(best_model_path):
        print(f"\nLoading best model with val_AP={best_val_ap:.4f}")
        model.load_state_dict(torch.load(best_model_path))
        # Save as final checkpoint
        final_ckpt = os.path.join(
            _MAGIC_ROOT,
            "checkpoints",
            f"checkpoint-{dataset_name}-seed{main_args.seed}{_run_suffix(main_args)}.pt",
        )
