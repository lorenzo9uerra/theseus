...
def get_default_cfg(args):
    ...
    set_dataset_cfg(cfg, args.dataset, args.csv_base_dir)
...
def set_dataset_cfg(cfg, dataset, csv_base_dir="./data/DARPA/"):
    cfg.dataset = CN()
    cfg.dataset.name = dataset
    for attr, value in DATASET_DEFAULT_CONFIG[cfg.dataset.name].items():
        setattr(cfg.dataset, attr, value)

    # Construct csv_dir from csv_base_dir + dataset name
    # Expand ~ to home directory if present
    csv_base_dir = os.path.expanduser(csv_base_dir)
    cfg.dataset.csv_dir = os.path.join(csv_base_dir, dataset, "")
...
def get_runtime_required_args(return_unknown_args=False, args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data/DARPA/",
        help="Base directory for CSV files. Dataset name will be appended (e.g., /path/to/DARPA/ + THEIA_E3/)",
    )
...
def update_task_paths_to_restart(cfg, subtask_concat_value=None):
    ...
    # Preserve the existing csv_dir instead of reconstructing it
    existing_csv_dir = cfg.dataset.csv_dir
    set_dataset_cfg(cfg, cfg.dataset.name, "")
    cfg.dataset.csv_dir = existing_csv_dir  # Restore the original csv_dir
    set_shortcut_variables(cfg)
...
def get_days_from_cfg(cfg):
    if cfg._test_mode:
        # Get the first available day from each non-empty split.
        days = [
            int(split_days[0].split("_")[-1])
            for split_days in [cfg.dataset.train_files, cfg.dataset.val_files, cfg.dataset.test_files]
            if split_days
        ]
    else:
        days = list(
