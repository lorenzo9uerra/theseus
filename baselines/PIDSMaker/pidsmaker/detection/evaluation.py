...
def standard_evaluation(cfg, evaluation_fn):
    ...
    log(
        f"Validation metrics - ADP: {best_epoch_info['adp_score']:.3f}, Discrimination: {best_epoch_info['discrimination']:.3f}, PR-AUC: {best_epoch_info['pr_auc']:.3f}, MCC: {best_epoch_info['mcc']:.3f}"
    )
...
def find_best_epoch_from_validation(val_losses_dir, cfg, evaluation_fn, tw_to_malicious_nodes):
    ...
    for model_epoch_dir in sorted_files:
        ...
        elif best_model_selection == "best_pr_auc":
            condition = stats["ap"] > best_metrics["pr_auc"]
        else:
        ...
        if condition:
            ...
            best_metrics["pr_auc"] = stats["ap"]
            best_metrics["mcc"] = stats.get("mcc", 0.0)
            best_metrics["best_epoch_dir"] = model_epoch_dir
            log(
                f"  -> New best: ADP={stats['adp_score']:.3f}, Discrimination={stats['discrimination']:.3f}, PR-AUC={stats['ap']:.3f}, MCC={stats.get('mcc', 0.0):.3f}"
            )
