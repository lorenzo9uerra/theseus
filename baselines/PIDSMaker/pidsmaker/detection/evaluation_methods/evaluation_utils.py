...
def _collect_process_node_scores(val_tw_path, cfg, *, use_dst_node_loss: bool):
    ...
    return node_scores, num_filtered


def get_max_benign_threshold(val_tw_path, cfg, use_dst_node_loss=True):
    """
    Set the threshold to the maximum validation score among benign process nodes.
    Attack, contaminated, and excluded nodes are removed from calibration.
    """
    (
        attack_nids,
        contaminated_nids,
    ...
    node_scores, num_filtered = _collect_process_node_scores(
        val_tw_path, cfg, use_dst_node_loss=use_dst_node_loss
    )
    calibration_excluded = set(attack_nids) | set(contaminated_nids) | set(excluded_nids)
    benign_scores = [
        score for node_id, score in node_scores.items() if node_id not in calibration_excluded
    ]

    log(
        "Max-benign threshold calibration: "
        f"filtered {num_filtered} non-process nodes, "
        f"excluded {len(calibration_excluded & set(node_scores))} attack/contaminated/excluded nodes, "
        f"using {len(benign_scores)} benign process nodes"
    )

    if benign_scores:
        threshold = float(max(benign_scores))
        log(f"Using max_benign_val_score threshold: {threshold:.5f}")
        return threshold

    if node_scores:
        threshold = float(max(node_scores.values()))
        log(
            "WARNING: No benign validation nodes available for calibration; "
            f"falling back to the maximum finite validation score ({threshold:.5f})"
        )
        return threshold

    log("WARNING: No validation node scores available; using infinite threshold")
    return float("inf")
...
def reduce_losses_to_score(losses: list[float], threshold_method: str):
    ...
    if (
        threshold_method == "max_benign_val_score"
        or threshold_method == "max_val_loss"
...
def get_ground_truth_nids(cfg):
    attack_nids, contaminated_nids, all_positive_nids, ground_truth_paths, excluded_nids = (
        get_ground_truth_with_labels(cfg)
    )
    return set(attack_nids), ground_truth_paths
