...
def compute_two_level_labels(nodes, attack_nids, contaminated_nids, all_positive_nids):
    """Compute labels for two-level evaluation: causal scope (attack+contaminated) vs attack chain (attack only)."""
    y_truth = []
    y_truth_strict = []
    mask_strict = []

    for nid in nodes:
        is_attack = nid in attack_nids
        is_contaminated = nid in contaminated_nids
        is_positive = nid in all_positive_nids

        y_truth.append(int(is_positive))

        if is_contaminated and not is_attack:
            y_truth_strict.append(0)
            mask_strict.append(False)
        else:
            y_truth_strict.append(int(is_attack))
            mask_strict.append(True)

    return y_truth, y_truth_strict, mask_strict
...
def evaluate_single_level(
    ...
    level_name = "Standard (attack+contaminated)" if prefix == "" else "Strict (attack only)"
    log(f"\n{'=' * 60}")
    ...
    if prefix == "":
        ...
        adp_img_file = os.path.join(out_dir, f"strict_adp_curve_{model_epoch_dir}.png")
        adp_score = plot_detected_attacks_vs_precision(
            pred_scores, nodes, node2attacks, y_truth, adp_img_file
        )
        discrim_scores = compute_discrimination_score(pred_scores, nodes, node2attacks, y_truth)
        ...
        if not skip_heavy_plots:
            score_dist_img_file = os.path.join(
                out_dir, f"strict_score_distribution_{model_epoch_dir}.png"
            )
            plot_anomaly_score_distribution(
                pred_scores,
                y_truth,
...
def main(
    ...
    **kwargs,
    ...
    # Compute two-level labels before deleting results
    y_truth, y_truth_strict, mask_strict = compute_two_level_labels(
        nodes, attack_nids, contaminated_nids, all_positive_nids
    )

    # Diagnostic: Check ground truth overlap with evaluation nodes
    nodes_set = set(nodes)
    ...
    if attack_chains:
        for chain_name, chain_data in attack_chains.items():
            # Use all_nids to include both attack and contaminated nodes
            attack2nodes[chain_name] = chain_data["all_nids"]
        log(
        ...
        for attack_id, metadata in attack_to_GPs.items():
            ...
            attack2nodes[attack_label] = metadata.get("all_nids", metadata["nids"])
    ...
    # Level 1: Causal Scope Evaluation (attack + contaminated = positive)
    stats_standard = evaluate_single_level(
        nodes=nodes,
        y_truth=y_truth,
        y_preds=y_preds,
        pred_scores=pred_scores,
        max_val_loss_tw=max_val_loss_tw,
        node2attacks=node2attacks,
        attack_to_GPs=attack_to_GPs,
    ...
    # Level 2: Attack chain (attack = positive, contaminated excluded)
    nodes_strict = [n for n, m in zip(nodes, mask_strict) if m]
    y_truth_strict_filtered = [y for y, m in zip(y_truth_strict, mask_strict) if m]
    y_preds_strict = [y for y, m in zip(y_preds, mask_strict) if m]
    pred_scores_strict = [s for s, m in zip(pred_scores, mask_strict) if m]
    max_val_loss_tw_strict = [t for t, m in zip(max_val_loss_tw, mask_strict) if m]

    # Create filtered node2attacks for strict evaluation (only attack nodes)
    node2attacks_strict = {n: attacks for n, attacks in node2attacks.items() if n in attack_nids}

    attack_to_GPs_strict = {}
    for att, data in attack_to_GPs.items():
        nids = data.get("nids", [])
        if any(nid in attack_nids for nid in nids):
            attack_to_GPs_strict[att] = data

    log(f"\nStrict Evaluation Set:")
    log(f"  Nodes after masking contaminated: {len(nodes_strict)} (from {len(nodes)})")
    log(f"  Contaminated nodes excluded: {sum(1 for m in mask_strict if not m)}")
    log(
        f"  Attacks (chains) after filtering contaminated-only: {len(attack_to_GPs_strict)} (from {len(attack_to_GPs)})"
    )

    stats_strict = evaluate_single_level(
        nodes=nodes_strict,
        y_truth=y_truth_strict_filtered,
        y_preds=y_preds_strict,
        pred_scores=pred_scores_strict,
        max_val_loss_tw=max_val_loss_tw_strict,
        node2attacks=node2attacks_strict,
    ...
    for k, v in stats_standard.items():
        final_stats[f"final_{k}"] = v

    # Add attack chain evaluation metrics with "final_strict_" prefix
    for k, v in stats_strict.items():
        final_stats[f"final_strict_{k}"] = v

    # Include unprefixed metrics for epoch selection compatibility
    for k, v in stats_standard.items():
        final_stats[k] = v
    ...
    log(f"\n{'=' * 60}")
    log("TWO-LEVEL EVALUATION SUMMARY")
    log(f"{'=' * 60}")
    log(f"Causal Scope (final_*): Contaminated nodes treated as positive")
    ...
    log(f"\nAttack Chain (final_strict_*): Contaminated nodes excluded from evaluation")
    log(f"  - Precision: {final_stats['final_strict_precision']:.4f}")
    log(f"  - Recall: {final_stats['final_strict_recall']:.4f}")
    log(f"  - F-Score: {final_stats['final_strict_fscore']:.4f}")
    log(f"  - ADP Score: {final_stats['final_strict_adp_score']:.4f}")
    log(f"  - Discrimination: {final_stats['final_strict_discrimination']:.4f}")
    log(f"{'=' * 60}")
