...
def test_node_level(
    ...
    # Magic codes
        ...
        # Extract node embeddings h_n from encoder
        ...
        if split == "val":
            ...
            x_val_sampled = x_embeddings[sample_indices]
            x_val_mean = x_val_sampled.mean(axis=0)
            x_val_std = x_val_sampled.std(axis=0)
            x_val_sampled = (x_val_sampled - x_val_mean) / x_val_std

            x_val_sampled = pd.DataFrame.from_records(x_val_sampled)

            n_neighbors = 10
            nbrs = NearestNeighbors(n_neighbors=n_neighbors)
            nbrs.fit(x_val_sampled)
            idx = list(range(x_val_sampled.shape[0]))
            random.shuffle(idx)
            try:
                sample = x_val_sampled.iloc[idx[: min(50000, x_val_sampled.shape[0])]]
                distances_val, _ = nbrs.kneighbors(
                    sample, n_neighbors=min(len(sample), n_neighbors)
                )
            except KeyError as e:
                log(f"KeyError encountered: {e}")
                log(f"Available columns in x_val_sampled: {x_val_sampled.columns}")
                raise
            mean_distance_val = distances_val.mean().mean()
            if mean_distance_val == 0:
                mean_distance_val = 1e-9

            val_distance_file = os.path.join(
                cfg.detection.gnn_training._magic_dir, "val_distance.txt"
            )
            with open(val_distance_file, "a") as f:
                f.write(f"{mean_distance_val}\n")

            # Compute magic_score for validation data (same as test)
            distances, _ = nbrs.kneighbors(x_embeddings, n_neighbors=n_neighbors)
            distances = distances.mean(axis=1)
            score = distances / mean_distance_val
            score = score.tolist()

            # Save node embeddings to CSV (for KNN outlier detection)
            ...
            # Use validation mean distance (not test) to avoid data leakage
            val_distance_file = os.path.join(
                cfg.detection.gnn_training._magic_dir, "val_distance.txt"
            )
            mean_distance_val = calculate_average_from_file(val_distance_file)
...
def main(cfg, model, val_data, test_data, epoch, split, logging=True):
    ...
    for dataset, split_name in splits:
        ...
        peak_inference_cpu_mem = max(peak_inference_cpu_mem, peak_inference_cpu_memory)
