...
def main(cfg):
    ...
    has_validation_split = any(len(dataset) > 0 for dataset in val_data)
    calibration_data = val_data if has_validation_split else train_data
    ...
    if not has_validation_split:
        log(
            "No validation split configured. Using benign training data for score "
            "calibration and selecting the last epoch for evaluation."
        )
    ...
    for epoch in range(0, num_epochs):
        if not use_few_shot or (use_few_shot and epoch > 0):
            ...
            peak_inference_cpu_memory = process.memory_info().rss / (1024**3)
            peak_train_cpu_mem = max(peak_train_cpu_mem, peak_inference_cpu_memory)
        ...
        if use_few_shot:
            ...
            for tuning_epoch in range(0, num_epochs_few_shot):
                ...
                if has_validation_split and patience_counter >= patience_few_shot:
        ...
        # Validation and Test - save predictions to disk for both val and test
        if (epoch + 1) % 2 == 0 or epoch == 0:
            ...
            if has_validation_split:
                # Track best model based on VALIDATION score only
                current_val_score = test_stats["val_score"]
                if current_val_score > best_val_score:
                    ...
                    patience_counter = 0
                    log(
                        f"[@epoch{epoch:02d}] New best validation score: {best_val_score:.5f}",
                        return_line=True,
                    )
                else:
                    patience_counter += 1
            ...
            if has_validation_split and patience > 0 and patience_counter >= patience:
                log(
                    f"Early stopping at epoch {epoch}: validation score hasn't improved for {patience} evaluations"
                )
                break

    # After training, load the best model (based on validation) and run final test evaluation
    if best_epoch_mode and has_validation_split and best_model is not None:
        log(f"Loading best model from epoch {best_epoch} (val_score={best_val_score:.5f})")
        model.load_state_dict(best_model)
        model.to_device(device)
        test_stats = inference_loop.main(
            cfg=cfg,
    ...
    wandb.log(
        {
            ...
            "val_score": round(best_val_score, 5) if has_validation_split else 0.0,
    ...
    return best_val_score if has_validation_split else 0.0
