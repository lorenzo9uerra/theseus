...
def load_data_list(path, split, cfg):
    ...
    for f in sorted(os.listdir(split_path)):
        filepath = os.path.join(split_path, f)
...
def compute_tgn_graphs(
    ...
    # For very large graphs, create a compact node mapping to reduce memory
    # Instead of allocating arrays of size max_node (e.g., 44M), we only allocate
    # for nodes that actually appear in the data
    use_compact_mapping = max_node > 10_000_000  # Use compact mapping for >10M nodes

    if use_compact_mapping:
        # Collect all unique nodes from all datasets
        all_nodes = set()
        for dataset in datasets:
            for data_list in dataset:
                for batch in data_list:
                    if batch.edge_index.numel() > 0:
                        all_nodes.update(batch.src.tolist())
                        all_nodes.update(batch.dst.tolist())

        # Also include nodes from full_data for neighbor lookups
        all_nodes.update(full_data.src.tolist())
        all_nodes.update(full_data.dst.tolist())

        all_nodes = sorted(all_nodes)
        compact_num_nodes = len(all_nodes)
        print(
            f"Using compact node mapping: {compact_num_nodes:,} active nodes (vs {max_node:,} max node ID)"
        )

        # Create bidirectional mapping
        node_to_compact = {node: idx for idx, node in enumerate(all_nodes)}
        compact_to_node = torch.tensor(all_nodes, dtype=torch.long)
        effective_num_nodes = compact_num_nodes
    else:
        node_to_compact = None
        compact_to_node = None
        effective_num_nodes = max_node

    print(f"Allocating TGN structures for {effective_num_nodes:,} nodes...")

    # Keep neighbor_loader on CPU during preprocessing to avoid GPU OOM
    # It will be moved to GPU later during actual training if needed
    neighbor_loader = LastNeighborLoader(
        effective_num_nodes, size=neighbor_size, directed=directed, device="cpu"
    )

    # Keep caches on CPU to avoid GPU OOM - they are only used for indexing
    node_feat_cache = torch.zeros((effective_num_nodes, node_feat_dim), device="cpu")
    node_type_cache = torch.zeros((effective_num_nodes, node_type_dim), device="cpu")
    assoc = torch.empty(effective_num_nodes, dtype=torch.long, device="cpu")

    for dataset in datasets:
        for data_list in dataset:
            for i, batch in enumerate(
                log_tqdm(data_list, desc="Computing TGN last neighbor graphs")
            ):
                # Keep batch on CPU for preprocessing to save GPU memory
                batch_edge_index = batch.edge_index.clone()

                # Skip empty batches (0 edges)
                if batch_edge_index.numel() == 0:
                    continue

                src, dst = batch_edge_index

                # Remap to compact indices if using compact mapping
                if use_compact_mapping:
                    src_compact = torch.tensor(
                        [node_to_compact[n.item()] for n in src], dtype=torch.long
                    )
                    dst_compact = torch.tensor(
                        [node_to_compact[n.item()] for n in dst], dtype=torch.long
                    )
                else:
                    src_compact = src
                    dst_compact = dst
                ...
                if insert_neighbors_before:
                    neighbor_loader.insert(src_compact, dst_compact)

                n_id = torch.cat([src_compact, dst_compact]).unique()
                ...
                if fix_tgn_neighbor_loader:
                    ...
                    real_src = full_data.src[e_id]
                    real_dst = full_data.dst[e_id]

                    # Convert full_data nodes to compact if needed
                    if use_compact_mapping:
                        real_src_compact = torch.tensor(
                            [node_to_compact.get(n.item(), -1) for n in real_src], dtype=torch.long
                        )
                        real_dst_compact = torch.tensor(
                            [node_to_compact.get(n.item(), -1) for n in real_dst], dtype=torch.long
                        )
                    else:
                        real_src_compact = real_src
                        real_dst_compact = real_dst

                    loader_src = n_id[edge_index[0]]
                    loader_dst = n_id[edge_index[1]]

                    match_dir1 = real_src_compact.eq(loader_src) & real_dst_compact.eq(loader_dst)
                    match_dir2 = real_src_compact.eq(loader_dst) & real_dst_compact.eq(loader_src)
                ...
                # All operations on CPU - use compact indices for caching
                assoc[n_id] = torch.arange(num_nodes)
                node_feat_cache[torch.cat([src_compact, dst_compact])] = torch.cat(
                    [batch.x_src, batch.x_dst]
                )
                node_type_cache[torch.cat([src_compact, dst_compact])] = torch.cat(
                    [batch.node_type_src, batch.node_type_dst]
                )

                if fix_buggy_orthrus_TGN:
                    x_src = torch.zeros((num_nodes, node_feat_dim))
                    x_dst = x_src.clone()
                    ...
                    # Use CPU device for graph_reindexer during preprocessing
                    orig_device = graph_reindexer.device
                    graph_reindexer.to("cpu")
                    ...
                    graph_reindexer.to(orig_device)
                ...
                # For reindexing, we need to use compact indices if using compact mapping
                if use_compact_mapping:
                    compact_original_n_id = torch.tensor(
                        [node_to_compact[n.item()] for n in batch.original_n_id], dtype=torch.long
                    )
                    batch.reindexed_original_n_id_tgn = assoc[compact_original_n_id].clone()
                    # n_id_tgn stores the original node IDs (converted back from compact)
                    batch.n_id_tgn = compact_to_node[n_id]
                    # reindexed_edge_index uses compact src/dst mapped through assoc
                    compact_edge_index = torch.stack([src_compact, dst_compact])
                    batch.reindexed_edge_index_tgn = assoc[compact_edge_index].clone()
                else:
                    batch.reindexed_original_n_id_tgn = assoc[batch.original_n_id].clone()
                    batch.n_id_tgn = n_id
                    batch.reindexed_edge_index_tgn = assoc[batch.edge_index].clone()

                batch.edge_index_tgn = edge_index

                # Index into full_data using e_id from neighbor_loader
                batch.msg_tgn = full_data.msg[e_id].clone()
                batch.t_tgn = full_data.t[e_id].clone()
                batch.edge_type_tgn = full_data.edge_type[e_id].clone()

                batch.node_type_tgn = node_type_cache[n_id].clone()
        ...
        # Force garbage collection after each dataset split (train/val/test)
        gc.collect()

    # Clean up large caches
    del node_feat_cache, node_type_cache, assoc, neighbor_loader
    gc.collect()
