...
def read_table(data_dir, table_name, columns=None):
    ...
    if os.path.exists(parquet_path):
        ...
        if columns:
            return pl.read_csv(csv_path, columns=columns, null_values=[""])
        return pl.read_csv(csv_path, null_values=[""])
    return None


def scan_table(data_dir, table_name):
    ...
    if os.path.exists(parquet_path):
        ...
        return pl.scan_csv(csv_path, null_values=[""])
    return None


def get_table_path(data_dir, table_name):
    """Get path to table file (Parquet preferred, CSV fallback)."""
    parquet_path = os.path.join(data_dir, f"{table_name}.parquet")
    csv_path = os.path.join(data_dir, f"{table_name}.csv")
...
def datetime_to_ns_time_with_tz(date, tz_name="US/Eastern"):
    """
    :param date: str   format: %Y-%m-%d %H:%M:%S
    :param tz_name: timezone name such as 'US/Eastern' or 'UTC'
    :return: nano timestamp
    """
    tz_name = (tz_name or "US/Eastern").strip()
...
def init_database_connection(cfg):
    # We don't use this function in this adaptation for Theseus' paper.
    log("Dummy database connection to remove dependency on psycopg2.")
    return None, None
...
def get_node_to_path_and_type(cfg):
    ...
    if not os.path.exists(out_file):
        ...
        def _read_node_table(table_name, columns=None):
        ...
        # Read file node table
        file_df = _read_node_table("file_node_table", ["index_id", "path"])
        if file_df is not None:
            for row in file_df.iter_rows(named=True):
                index_id = int(row["index_id"])
                path = row.get("path", "") or ""
                node_to_path_type[index_id] = {"path": str(path), "type": "file"}
            del file_df

        # Read netflow node table
        netflow_df = _read_node_table("netflow_node_table")
        if netflow_df is not None:
            cols = netflow_df.columns
            for row in netflow_df.iter_rows(named=True):
                index_id = int(row["index_id"])
                # Handle different column naming conventions
                src_addr = row.get("src_addr") or row.get("local_ip") or ""
                dst_addr = row.get("dst_addr") or row.get("remote_ip") or ""
                src_port = row.get("src_port") or row.get("local_port") or ""
                dst_port = row.get("dst_port") or row.get("remote_port") or ""
        ...
        # Read subject node table
        subject_df = _read_node_table("process_node_table")
        if subject_df is not None:
            for row in subject_df.iter_rows(named=True):
                index_id = int(row["index_id"])
                path = row.get("path", "") or ""
                cmd = row.get("cmd") or row.get("cmd_line") or ""
                node_to_path_type[index_id] = {"path": str(path), "type": "subject", "cmd": cmd}
            del subject_df
...
def log_dataset_stats(datasets):
    def log_helper(label, dataset):
        if len(dataset) == 0:
            log(f"{label} num graphs: 0")
            log(f"{label} edges | mean: 0 | min: 0 | max: 0")
            log(f"{label} nodes | mean: 0 | min: 0 | max: 0")
            log("")
            return
...
def set_seed(cfg):
    if cfg.detection.gnn_training.seed:
