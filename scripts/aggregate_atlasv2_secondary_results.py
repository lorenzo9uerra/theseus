...
def _format_markdown(
    *,
    ...
    lines = [
        ...
        "- revised process labels are matched exactly by Carbon Black process UUID",
        "",
        "## Main Table",
        "",
    ...
    lines.extend(
        [
            ...
            "## Caveats",
            "",
            "- ATLASv2 is a secondary benchmark. Its graph is reconstructed from EDR telemetry, and each host effectively contains a single engagement, so these results should not be interpreted like the temporally separated E3 benchmarks.",
            "- No attack-bearing validation stage is available, so Theseus and Velox use training-derived thresholds.",
