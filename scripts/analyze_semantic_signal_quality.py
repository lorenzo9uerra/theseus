#!/usr/bin/env python3
"""Analyze semantic signal quality inputs for processed node attributes."""
...
# Accept `subject_node_table` for legacy external layouts.
PROCESS_TABLE_CANDIDATES = ("process_node_table", "subject_node_table")
NETFLOW_FIELDS = ["src_addr", "src_port", "dst_addr", "dst_port"]
...
class DatasetStats:
    ...
    def overall_entropy(self) -> float:
        """Average bits per filled attribute slot."""
        total_filled = 0
        weighted_sum = 0.0
...
def _median_from_histogram(length_histogram: Counter[int], total_count: int) -> float:
    if total_count == 0:
        return 0.0
    midpoint = total_count // 2
    cumulative = 0
    for length in sorted(length_histogram):
        cumulative += length_histogram[length]
...
def analyze_field(
    ...
    for text in non_empty:
        ...
        token_length_histogram[token_count] += 1
        total_token_length += token_count
    ...
    return FieldStats(
        ...
        avg_tokens=total_token_length / n,
        median_tokens=_median_from_histogram(token_length_histogram, n),
...
def analyze_netflow(
    df: pl.DataFrame, require_complete_endpoint: bool = True
) -> FieldStats | None:
    ...
    if require_complete_endpoint:
        non_empty_mask = pl.all_horizontal(
            [pl.col(c).str.strip_chars() != "" for c in cols]
        )
    else:
...
def analyze_node_type(
    ...
    fields: list[str],
    data_root: str,
    require_complete_netflow: bool = True,
) -> NodeTypeStats | None:
    """Load Parquet/CSV and analyze specified fields."""
    data_dir = os.path.join(data_root, dataset)
    columns = NETFLOW_FIELDS if node_type == "netflow" else fields
    try:
        df = read_node_table(data_dir, table_name, columns=columns)
    except Exception:
    ...
    if node_type == "netflow":
        stats = analyze_netflow(df, require_complete_netflow)
...
def analyze_process_node_type(dataset: str, data_root: str) -> NodeTypeStats | None:
    for table_name in PROCESS_TABLE_CANDIDATES:
        stats = analyze_node_type(
            dataset, "process", table_name, ["path", "cmd"], data_root
        )
...
def analyze_dataset(
    ...
    process = analyze_process_node_type(dataset, data_root)
    file = analyze_node_type(dataset, "file", "file_node_table", ["path"], data_root)
    netflow = analyze_node_type(
        dataset,
        "netflow",
        "netflow_node_table",
        [],
        data_root,
        require_complete_netflow=require_complete_netflow,
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze semantic signal quality inputs for processed datasets"
    )
    parser.add_argument(
        "dataset", nargs="*", help="Dataset name(s) (e.g., THEIA_E3 TRACE_E3)"
    )
    parser.add_argument(
        "--data-root",
        default=os.path.join(PROJECT_ROOT, "data", "DARPA"),
    ...
    parser.add_argument(
        "--allow-partial-netflow",
        action="store_true",
        help="Count a netflow description as filled when any endpoint field exists",
    )
        ...
        try:
            results.append(
                analyze_dataset(
                    ...
                    require_complete_netflow=not args.allow_partial_netflow,
                )
            )
