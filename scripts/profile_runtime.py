#!/usr/bin/env python3
"""Profile Theseus runtime at representative E3 graph sizes."""
...
def profile_embedding_generation(
    ...
    result = RuntimeProfileResult("embedding_generation")
...
def profile_graph_construction(
    ...
    result = RuntimeProfileResult("graph_construction")
...
def profile_inference(
    model, graph, config, num_samples: int = 10
) -> RuntimeProfileResult:
    """Profile inference including graph-to-GPU transfer."""
    result = RuntimeProfileResult("inference")
...
def profile_end_to_end(
    ...
    result = RuntimeProfileResult("end_to_end")
    device = config.device
...
def print_runtime_profile(
    results: list[RuntimeProfileResult],
    graph_stats: dict | None = None,
    scenario: str = "Synthetic",
):
    print("\n" + "=" * 90)
    print(f"RUNTIME PROFILE ({scenario})")
    print("=" * 90)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Profile Theseus runtime at representative E3 graph sizes.",
    ...
    log(f"Profiling Theseus runtime for {args.dataset}")
    ...
    log("Running runtime measurements...")

    embedding_result = profile_embedding_generation(node_ids, config, args.num_samples)
    embedding_result.num_nodes = len(node_ids)

    construction_result = profile_graph_construction(
        edge_arrays,
        day,
        config,
        node_embeddings,
    ...
    inference_result = profile_inference(model, sample_graph, config, args.num_samples)
    inference_result.num_nodes = graph_stats["num_nodes"]

    e2e_result = profile_end_to_end(
        num_nodes,
        num_edges,
        DARPA_TC_EVENTS,
        base_timestamp,
