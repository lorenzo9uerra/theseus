...
from utils.constants.graph_events import NODE_TYPES, get_dataset_event_types
...
def _format_day_list_signature(days):
    if not days:
        return "none"
    return "-".join(str(int(day)) for day in days)


def _selected_day_signature(config):
    return (
        f"tr{_format_day_list_signature(config.dataset_info.train_days)}"
        f"_va{_format_day_list_signature(config.dataset_info.val_days)}"
        f"_te{_format_day_list_signature(config.dataset_info.test_days)}"
    )
...
def ensure_partitioned_event_parquet(config, event_type_set):
    ...
    parquet_path = os.path.join(
        data_dir, f"event_table_preprocessed_{_selected_day_signature(config)}.parquet"
    )
...
def get_training_nodes_from_csv(config):
    ...
    parquet_path = ensure_partitioned_event_parquet(
        config, set(get_dataset_event_types(config.dataset))
    )
    events_scan = pl.scan_parquet(parquet_path).select(
        ["event_day", "src_index_id", "dst_index_id"]
    )
...
def get_cache_filename(dataset_name, config):
    ...
    components.append(_selected_day_signature(config))
    token_weighting_mode = get_token_weighting_mode(config)
