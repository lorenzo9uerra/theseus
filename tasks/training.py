...
@timed_execution
def train(config, train_data, val_data, test_data, ground_truth=None):
    ...
    reconstruction_loss_reduction = config.reconstruction_loss_reduction
    has_validation_split = bool(val_data)
    ...
    if not has_validation_split:
        log(
            "No validation split provided. Training will run for the configured "
            "number of epochs without validation-based early stopping."
        )
    ...
    for epoch in (
        ...
        model.eval()
        val_ap, val_loss, threshold = validate(model, val_data, config, ground_truth)
    ...
    if has_validation_split:
        log(f"Best validation AP: {best_val_ap:.4f} at epoch {best_epoch}")
    else:
        log(f"Saved final checkpoint at epoch {best_epoch} (no validation split).")
