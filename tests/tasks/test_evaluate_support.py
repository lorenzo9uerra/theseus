...
def test_select_max_benign_threshold_uses_benign_scores_only():
    threshold = select_max_benign_threshold(
        np.array([0.2, 0.5, 0.8]), np.array([0, 1, 0])
    )

    assert threshold == pytest.approx(0.8)


def test_threshold_predictions_use_exclusive_rule():
...
def test_compute_binary_metrics_returns_expected_schema():
    metrics = compute_binary_metrics(
        np.array([0, 0, 1, 1]), np.array([0, 1, 0, 1]), "final_test", "confusion_matrix"
    )
...
def test_build_node_to_attack_mappings_keeps_only_attack_nodes():
    attack_map = build_node_to_attack_mappings(
        {"A-1": {"nids": [1], "contaminated_nids": [2, 3]}}
    )

    assert attack_map == {1: {"A-1"}}
