...
def test_main_training_path_dispatches_tasks(monkeypatch):
    ...
    monkeypatch.setattr("main.set_seed", lambda seed: call_log.append(("set_seed", seed)))
    monkeypatch.setattr(
        "main.build_graphs", lambda cfg: (graphs, ground_truth)
    )
    monkeypatch.setattr(
        "main.train",
        lambda cfg, train_data, val_data, test_data: (
            call_log.append(("train", train_data, val_data, test_data)) or dummy_model
        ),
    )
    monkeypatch.setattr(
        "main.evaluate",
        lambda model, val_data, test_data, cfg, gt: (
            call_log.append(("evaluate", model, val_data, test_data, gt)) or metrics
        ),
    )
    monkeypatch.setattr(
        "main.wandb.log", lambda logged_metrics: call_log.append(("wandb.log", logged_metrics))
    )
    monkeypatch.setattr(
    ...
    assert call_log == [
        ("set_seed", 123),
        ("train", ["train_graph"], ["val_graph"], ["test_graph"]),
        ("evaluate", dummy_model, ["val_graph"], ["test_graph"], ground_truth),
        ("wandb.log", metrics),
    ]
